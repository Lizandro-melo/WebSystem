...
import {
  ...
  SubcategoriaTiModels,
  GrupoClassificacaoTiModels,
} from "@/lib/models";
import {
  cn,
...
function TicketsChamados() {
  ...
  const { data: solicitacoes } = useQuery({
    queryKey: ["lista_solicitacoes_manutencao"],
    queryFn: async () => {
      if (acessos?.rolesTI?.delegado && acessos?.rolesTI && acessos)
        return await axios
          .get(`${host}/suporte/find/solicitacao`, configToken)
          .then((response) => {
        ...
        return await axios
          .get(
            `${host}/suporte/find/solicitacao/by?id=${user?.fkAuth!}`,
            configToken,
          )
          .then((response) => {
            const solicitacoes: SolicitcaoTiDTO[] = response.data;
  ...
  const joinSolicitacao = async (idSolicitacao: number | undefined) => {
    ...
    await axios
      .get(
        `${host}/suporte/find/solicitacao/exatc?id=${idSolicitacao}`,
        configToken,
      )
      .then((response) => {
  ...
  return (
    <div className="h-full w-[20%]  shadow-lg bg-[--color-tec] border-l-2 border-y-2 border-slate-600 dark:border-slate-600">
      <div className="flex justify-between px-5 items-center w-full h-[10%] bg-[--color-tec] border-b-2 border-slate-600 dark:border-slate-600">
              ...
              .map((solicitacao: SolicitcaoTiDTO, i: number) => {
                return (
                  <button
                    ...
                    className={cn(
                      "bg-[--color-tec] px-2 h-[80px] relative flex items-center text-black dark:text-white font-medium justify-between transition-all w-full border-slate-600 dark:border-slate-600 border-b-2",
                      solicitacao.select && "outline  outline-offset-[-4px]",
                    )}
                  >
                    <Avatar>
                      <AvatarImage
                        src={
                          !solicitacao.solicitante.infoPessoais.dirFoto
                    ...
                    <span className="text-xs text-black dark:text-white font-medium whitespace-nowrap">
                      {alterNomeCompletoParaNomeSobrenome(
                        solicitacao.solicitante.infoPessoais.nomeCompleto,
                      )}
                    </span>
                    <span className="smallSpan text-black dark:text-white font-medium absolute right-2 top-1">
                      {formatDateTimeUser(solicitacao.dataHora)}
                    </span>
                    <span className="smallSpan text-black dark:text-white font-medium absolute right-2 bottom-1">
                      #{solicitacao.id}
                    </span>
                  </button>
                );
...
function TicketsSolicitacao() {
  ...
  return (
    <div
      className={cn(
        "w-[80%] h-full  border-x-2 border-t-2 border-slate-600 dark:border-slate-600 ",
      )}
    >
      {selectSolicitacao.solicitacaoSelect && (
        <>
          <header
            className={cn(
              "w-full h-[10%] gap-5 bg-[--color-tec] border-b-2 border-slate-600 dark:border-slate-600 flex items-center justify-between px-5",
            )}
          >
            <div className="flex items-center justify-start gap-5">
              ...
              <div className={"flex flex-col text-xs gap-2"}>
                <span>
                  {alterNomeCompletoParaNomeSobrenome(
                    selectSolicitacao.solicitacaoSelect.solicitante.infoPessoais
                      .nomeCompleto,
                  )}
                </span>
                <span>
          ...
          >
            <div
              className="w-full p-5  overflow-auto scrowInvivel"
              ref={refDivMensagens}
            >
              <div className="flex gap-6 flex-col w-full relative">
                ...
                >
                  {selectSolicitacao.solicitacaoSelect.solicitante.infoPessoais
                    .fkAuth !== user?.fkAuth ? (
                    <div className={cn("flex gap-5 w-full", "justify-start")}>
                      <div>
                        <Avatar>
                          <AvatarImage
                            src={
                              !selectSolicitacao.solicitacaoSelect.solicitante
                                ...
                                ? "https://placehold.co/600?text=Foto"
                                : selectSolicitacao.solicitacaoSelect
                                    .solicitante.infoPessoais.dirFoto
                            }
                            alt="fotoColaborador"
                          />
                        </Avatar>
                      </div>
                      <div className="w-[50%] p-5 bg-[--color-tec] rounded-xl relative flex flex-col gap-3">
                        ...
                        <span
                          ...
                          dangerouslySetInnerHTML={{
                            __html:
                              selectSolicitacao.solicitacaoSelect.ocorrencia,
                          }}
                        ></span>
                        <span className="smallSpan absolute right-4 bottom-1">
                          {
                            selectSolicitacao.solicitacaoSelect.dataHora!.split(
                              "T",
                            )[1]
                          }
                        </span>
  ...
  return (
    <>
      ...
      <form
        ...
        className={cn(
          "w-full z-20 py-3 bg-[--color-tec] flex items-center justify-start border-t-2 border-slate-600 dark:border-slate-600",
        )}
      >
        <button
          onClick={() => {
          ...
          className={
            "w-[5%] h-full flex items-end justify-center cursor-pointer group"
          }
        >
          <Paperclip
            className={
              "dark:stroke-slate-400 stroke-slate-600 group-hover:stroke-white"
            }
          />
        </button>
        <div
        ...
        >
          <SendHorizonal
            className={
              "dark:stroke-slate-400 stroke-slate-600 group-hover:stroke-white group-active:scale-90 transition-all"
            }
          />
        </button>
      </form>
